...
describe('StatsQueries', () => {
  ...
  describe('getEvents', () => {
    it('Should return a query', () => {
      ...
      expect(result.index).toBe('stats');
      expect(result.body.query.bool.must[1].range.date.gte).toBe(START_DATE.getTime());
      expect(result.body.query.bool.must[1].range.date.lte).toBe(STOP_DATE.getTime());
    })
  })
});
