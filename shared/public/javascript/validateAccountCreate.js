...
export function validateAccountCreate(element) {
  ...
  const inputCheckboxRoles = Object.values(document.querySelectorAll('input[name="roles[]"]'));
  ...
  for (let i = 0; i < inputs.length; ++i) {
    ...
    inputs[i].addEventListener('input', function(event){		
      ...
      if (inputs[i].checked) {
        ...
        const countCheckedRole = inputCheckboxRoles.filter(({ checked }) => checked === true).length;
        if (countCheckedRole === 0 ) {
          inputCheckboxRoles.forEach(chkboxRole => {
            inputInvalid(chkboxRole);
          });
          document.querySelector('.roles-error').innerText = 'Veuillez renseigner au moins un rôle';
        }
      }	
    }, false);
